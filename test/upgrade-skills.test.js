...
import { buildUpgradePlan, createProject, readJournal } from "../src/index.js";
...
describe("upgrade skill reconciliation", () => {
  ...
  it("removes an upstream-deleted skill file from canonical, baseline, and projections", async () => {
    ...
    const plan = await buildUpgradePlan(root, { allowSkillRemoval: true, allowNetwork: true });
    assert.equal(plan.canApply, true, plan.conflicts.join("\n"));
  ...
  it("rejects incomplete marker keys and unrecorded projection directories", async () => {
    ...
    const canonical = path.join(root, ".agentic", "skills", "verify", "SKILL.md");
    await writeFile(canonical, `${await readFile(canonical, "utf8")}\n<!-- local extension -->\n`);
    await applyWithVerifier(await buildUpgradePlan(root, { allowNetwork: true }), async () => ({ ok: true }));
    ...
    const missingKey = await buildUpgradePlan(root, { allowNetwork: true });
    ...
    const manifestMismatch = await buildUpgradePlan(root, { allowNetwork: true });
    ...
    const missingGlobalHashes = await buildUpgradePlan(root, { allowNetwork: true });
    ...
    const extraDirectory = await buildUpgradePlan(root, { allowNetwork: true });
