...
When preserving a legacy contract, every executable ticket still requires at least one nonblank exact command and exemption markers remain exact strings. This compatibility rule validates history; it does not authorize generating a new validator or ticket pack.

Wayfinder is optional input, not a prerequisite. Invoke it only when a still-open product or architecture choice can materially change the destination or route.
